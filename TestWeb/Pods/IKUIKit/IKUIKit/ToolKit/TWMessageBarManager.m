...
#import <IKUIKit/TWMessageBarManager.h>
...
- (void)showMessageWithTitle:(NSString *)title
                 description:(NSString *)description
                        type:(TWMessageBarMessageType)type
...
+ (void)showToastWithMsg:(NSString *)msg {
    [[TWMessageBarManager sharedInstance] showMessageWithTitle:msg
                                                   description:nil
...
+ (void)showToastWithMsg:(NSString *)msg isLandscape:(BOOL)isLandscape {
    [[TWMessageBarManager sharedInstance] showMessageWithTitle:msg
                                                   description:nil
                                                          type:TWMessageBarMessageTypeError
                                                      duration:2.5f
...
- (void)showMessageWithTitle:(NSString *)title
    ...
    [self showMessageWithTitle:title
                   description:description
                          type:type
                      duration:[TWMessageBarManager durationForMessageType:type]
...
- (void)showMessageWithTitle:(NSString *)title
    ...
    messageView.statusBarStyle = statusBarStyle;
    messageView.statusBarHidden = statusBarHidden;
    messageView.isLandscape = isLandscape;
    
    [self messageWindowView];
    if (isLandscape) {
        CGRect frame = [UIScreen mainScreen].bounds;
        self.messageWindow.transform = CGAffineTransformMakeRotation(M_PI_2);
        self.messageWindow.frame = frame;
    }
    [[self messageWindowView] addSubview:messageView];
    [[self messageWindowView] bringSubviewToFront:messageView];
        ...
        if ([subview isKindOfClass:[TWMessageView class]]) {
            ...
            if (animated) {
                [UIView animateWithDuration:kTWMessageBarManagerDismissAnimationDuration animations:^{
                    currentMessageView.frame = CGRectMake(currentMessageView.frame.origin.x, -currentMessageView.frame.size.height,
                                                          currentMessageView.frame.size.width, currentMessageView.frame.size.height);
...
- (void)showNextMessage {
    if ([self.messageBarQueue count] > 0) {
        ...
        UIPanGestureRecognizer *swipe = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(itemSelected:)];
        [messageView addGestureRecognizer:swipe];
        ...
        if (messageView) {
            ...
            [UIView
             animateWithDuration:kTWMessageBarManagerDismissAnimationDuration
             animations:^{
                 [messageView setFrame:CGRectMake(messageView.frame.origin.x, messageView.frame.origin.y + [messageView height],
                                                  [messageView width], [messageView height])]; // slide down
             }];
...
- (void)itemSelected:(id)sender {
    ...
    if (messageView && ![messageView isHit]) {
        ...
        [UIView animateWithDuration:kTWMessageBarManagerDismissAnimationDuration animations:^{
            [messageView setFrame:CGRectMake(messageView.frame.origin.x, messageView.frame.origin.y - [messageView height], [messageView width],
...
- (TWMessageBarViewController *)messageBarViewController {
    if (!self.messageWindow) {
        ...
        self.messageWindow.windowLevel = UIWindowLevelNormal + 1;
...
+ (void)initialize {
    if (self == [TWMessageView class]) {
        ...
        kTWMessageViewDescriptionFont = [UIFont systemFontOfSize:12.0];
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([self.delegate respondsToSelector:@selector(styleSheetForMessageView:)]) {
        ...
        if (self.titleString && !self.descriptionString) {
            xOffset = ceil(rect.size.width * 0.5) - ceil(titleLabelSize.width * 0.5);
            yOffset = ceil((rect.size.height - 20) * 0.5) - ceil(titleLabelSize.height * 0.5);
        ...
        if ([[UIDevice currentDevice] isRunningiOS7OrLater]) {
            ...
            [[self titleColor] set];
            if (yOffset + titleLabelSize.height >= 64) {
                yOffset -= 6;
            }
...
- (CGFloat)height {
    //  CGSize titleLabelSize = [self titleSize];
    //  CGSize descriptionLabelSize = [self descriptionSize];
    //  return MAX((kTWMessageViewBarPadding * 2) + titleLabelSize.height +
    //                 descriptionLabelSize.height + [self statusBarOffset],
    //             (kTWMessageViewBarPadding * 2) + kTWMessageViewIconSize +
...
- (CGFloat)width
{
    ...
    if (self.isLandscape) {
        w = [UIApplication sharedApplication].keyWindow.frame.size.height;
    } else {
        w = [self statusBarFrame].size.width;
    }
...
- (CGRect)statusBarFrame {
    CGRect windowFrame = NSFoundationVersionNumber <= NSFoundationVersionNumber_iOS_7_1 ? [self orientFrame:[UIApplication sharedApplication].keyWindow.frame]
    : [UIApplication sharedApplication].keyWindow.frame;
    CGRect statusFrame = NSFoundationVersionNumber <= NSFoundationVersionNumber_iOS_7_1 ? [self orientFrame:[UIApplication sharedApplication].statusBarFrame]
...
- (void)didChangeDeviceOrientation:(NSNotification *)notification {
    ...
    //                 [self statusBarFrame].size.width, self.frame.size.height);
...
+ (void)initialize {
    if (self == [TWDefaultMessageBarStyleSheet class]) {
        // define color by gallop
        //    UIColor *color = [UIColor colorWithRed:165 / 255.0
        //                                     green:142 / 255.0
        //                                      blue:226 / 255.0
        //                                     alpha:1.0];
        UIColor *color = mIKColor_7;
        
        // Colors (background)
        kTWDefaultMessageBarStyleSheetErrorBackgroundColor = color;
        //    [UIColor
        //        colorWithRed:1.0
        //               green:0.611
        //                blue:0.0
        //               alpha:kTWMessageBarStyleSheetMessageBarAlpha];  // orange
        kTWDefaultMessageBarStyleSheetSuccessBackgroundColor = color;
        //        [UIColor colorWithRed:0.0f
        //                        green:0.831f
        //                         blue:0.176f
        //                        alpha:kTWMessageBarStyleSheetMessageBarAlpha];  //
        //                        green
        kTWDefaultMessageBarStyleSheetInfoBackgroundColor = color;
        ...
        // Colors (stroke)
        kTWDefaultMessageBarStyleSheetErrorStrokeColor = color;
        //        [UIColor colorWithRed:0.949f
        //                        green:0.580f
        //                         blue:0.0f
        //                        alpha:1.0f];  // orange
        kTWDefaultMessageBarStyleSheetSuccessStrokeColor = color;
        //        [UIColor colorWithRed:0.0f
        //                        green:0.772f
        //                         blue:0.164f
        //                        alpha:1.0f];  // green
        kTWDefaultMessageBarStyleSheetInfoStrokeColor = color;
        //        [UIColor colorWithRed:0.0f
        //                        green:0.415f
        //                         blue:0.803f
        //                        alpha:1.0f];  // blue
