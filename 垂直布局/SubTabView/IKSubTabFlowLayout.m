...
@interface IKSubTabFlowLayout ()

@property (nonatomic, strong)       NSMutableArray * widthOfColumns; // 用于记录每一列布局到的宽度
@property (nonatomic, strong)       NSMutableArray * itemsAttributes;// 用于保存所有item的属性 (frame)

@end


@implementation IKSubTabFlowLayout
...
-(void)prepareLayout {
    ...
    for (NSInteger i = 0; i<count; i++) {
        ...
        NSIndexPath * indexPath = [NSIndexPath indexPathForItem:i inSection:0];
            // 动态的获取宽度
        if ([self.delegate respondsToSelector:@selector(subTabFlowLayout:widthForItemAtIndexPath:)]) {
            itemWidth = [self.delegate subTabFlowLayout:self widthForItemAtIndexPath:indexPath];
        }
        UICollectionViewLayoutAttributes * attr = [UICollectionViewLayoutAttributes layoutAttributesForCellWithIndexPath:indexPath];
        
        attr.frame = CGRectMake(originX, originY, itemWidth, itemHeight);
        NSLog(@"cjw %@",NSStringFromCGRect(attr.frame));
            // 保存 item 的属性 到数组中
        [_itemsAttributes addObject:attr];
            // 更新布局到的一列(最短列) 的高度
        _widthOfColumns[index] = @(originX + itemWidth + self.minimumLineSpacing);
    }
    if ([self.delegate respondsToSelector:@selector(subTabFlowLayout:needMaxWidth:)]) {
        ...
        [self.delegate subTabFlowLayout:self needMaxWidth:width];
    }
}

- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    return _itemsAttributes[indexPath.row];
}
