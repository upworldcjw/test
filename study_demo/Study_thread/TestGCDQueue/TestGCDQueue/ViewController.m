...
@end
 ...
 void *kSerial = &kSerial;
 void *kConcurrent = &kConcurrent;
void *kMain = &kMain;

@implementation ViewController

- (void)viewDidLoad {
    dispatch_queue_set_specific(dispatch_get_main_queue(), kMain, kMain, nil);
    if (!_serialQueue){
        static NSString *queueSerialName = @"test.serial.queue";
        _serialQueue =
        dispatch_queue_create([queueSerialName UTF8String], DISPATCH_QUEUE_SERIAL);
        dispatch_queue_set_specific(_serialQueue, kSerial, kSerial, nil);
    }
    if (!_concurrentQueue){
        ...
        dispatch_queue_set_specific(_concurrentQueue, kConcurrent, kConcurrent, nil);
    }
...
- (void)printLog {
    if (dispatch_get_specific(kMain)) {
        NSLog(@"in mainQueue");
    }
    if (dispatch_get_specific(kConcurrent) == kConcurrent) {
        NSLog(@"in concurrentQueue");
    }
    if (dispatch_get_specific(kSerial) == kSerial) {
        NSLog(@"in serialQueue");
    }
}
- (void)testMainThread {
